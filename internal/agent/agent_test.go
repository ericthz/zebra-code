...
func TestSkillCreatorOutputsToCorrectDirectory(t *testing.T) {
	...
	// agent creates the new skill → verify it lands in .zebracode/skills/, not root

	workDir := t.TempDir()
	skillsDir := filepath.Join(workDir, ".zebracode", "skills")
...
New skills MUST be created under the .zebracode/skills/ directory.
The full path should be .zebracode/skills/<skill-name>/SKILL.md.
`), 0o644)
	...
	client := &dynamicMock{handlers: []func([]conversation.Message) []llm.StreamEvent{
		...
		// Round 2: user describes, agent creates the skill in .zebracode/skills/
		func(msgs []conversation.Message) []llm.StreamEvent {
			last := msgs[len(msgs)-1]
...
`,
	...
	// THE KEY CHECK: new skill was created inside .zebracode/skills/, NOT in project root
...
func TestRealSkillsLoadAndRunSimulation(t *testing.T) {
	...
	if _, err := os.Stat(skillsDir); os.IsNotExist(err) {
		t.Skip("No .zebracode/skills directory")
	}
