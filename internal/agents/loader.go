...
// ZEBRACODE_VERIFICATION_AGENT 环境变量控制，需要时才加进来。
func getBuiltinSpecs() map[string]SubAgentSpec {
	...
	if os.Getenv("ZEBRACODE_VERIFICATION_AGENT") == "true" {
...
func (l *AgentLoader) LoadAll() error {
	...
	if home != "" {
		l.loadDir(filepath.Join(home, ".zebracode", "agents"), "user")
	}

	if l.workDir != "" {
		l.loadDir(filepath.Join(l.workDir, ".zebracode", "agents"), "project")
	}
	...
	for _, entry := range entries {
		...
		if err != nil {
			...
			if l.ErrorWriter != nil {
				fmt.Fprintf(l.ErrorWriter, "[zebracode] agent definition skipped — %s\n", msg)
			}
			continue
		}
		def.Source = source
