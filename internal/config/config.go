...
func LoadConfig(path string) (*AppConfig, error) {
	...
	candidates := []string{
		filepath.Join(home, ".zebracode", "config.yaml"),
		filepath.Join(wd, ".zebracode", "config.yaml"),
		filepath.Join(wd, ".zebracode", "config.local.yaml"),
	}
	...
	if merged == nil {
		return nil, &ConfigError{Message: "No config file found. Expected .zebracode/config.yaml in project or ~/.zebracode/config.yaml"}
	}
