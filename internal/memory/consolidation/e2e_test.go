...
// 需要环境变量 ZEBRACODE_TEST_API_KEY 和 ZEBRACODE_TEST_BASE_URL
func TestE2E_ConsolidationMergesDuplicates(t *testing.T) {
	apiKey := os.Getenv("ZEBRACODE_TEST_API_KEY")
	baseURL := os.Getenv("ZEBRACODE_TEST_BASE_URL")
	model := os.Getenv("ZEBRACODE_TEST_MODEL")
	if apiKey == "" {
		t.Skip("ZEBRACODE_TEST_API_KEY not set, skipping E2E test")
	}
	if baseURL == "" {
	...
	// 构造测试目录
	dir := t.TempDir()
	memDir := filepath.Join(dir, ".zebracode", "memory")
