...
// ~/.zebracode/memory 上，与其它并行测试互相干扰。
...
func TestGetAutoMemPath(t *testing.T) {
	t.Setenv("ZEBRACODE_REMOTE_MEMORY_DIR", "")
	projectRoot := filepath.Join(t.TempDir(), "project")
	os.MkdirAll(projectRoot, 0o755)
	path := GetAutoMemPath(projectRoot)
	wantSuffix := filepath.Join(".zebracode", "memory") + string(filepath.Separator)
...
func TestGetAutoMemPathRespectsOverride(t *testing.T) {
	...
	t.Setenv("ZEBRACODE_REMOTE_MEMORY_DIR", overrideDir)
