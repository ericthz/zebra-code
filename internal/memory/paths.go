...
// project root. Shape: <projectRoot>/.zebracode/memory/
//
...
//
// Resolution order:
//  1. ZEBRACODE_REMOTE_MEMORY_DIR env var — used as-is (escape hatch for
//     CI/container scenarios where memory should live elsewhere)
//  2. <projectRoot>/.zebracode/memory
func GetAutoMemPath(projectRoot string) string {
	if override := os.Getenv("ZEBRACODE_REMOTE_MEMORY_DIR"); override != "" {
	...
	return filepath.Join(abs, ".zebracode", "memory") + string(filepath.Separator)
}
...
// ~/.zebracode/memory/. Used for type=user / type=feedback memories that
...
func GetUserAutoMemPath() string {
	...
	return filepath.Join(home, ".zebracode", "memory") + string(filepath.Separator)
}

// GetUserAutoMemEntrypoint returns the path to ~/.zebracode/memory/MEMORY.md.
