...
//  1. User global: ~/.zebracode/ZEBRACODE.md, ~/.zebracode/AGENTS.md
//  2. Project: walk from git root down to workDir, picking up ZEBRACODE.md
//     and AGENTS.md in each directory (so the file closest to cwd wins)
//  3. workDir/.zebracode/INSTRUCTIONS.md (legacy)
//  4. workDir/ZEBRACODE.local.md (private local override)
//
// @-include directives:
...
func DiscoverInstructions(workDir string) []InstructionSource {
	...
	if home, err := os.UserHomeDir(); err == nil {
		add(&sources, seen, filepath.Join(home, ".zebracode", "ZEBRACODE.md"), projectRoot)
		add(&sources, seen, filepath.Join(home, ".zebracode", "AGENTS.md"), projectRoot)
	}
	for _, dir := range projectInstructionDirs(workDir) {
		add(&sources, seen, filepath.Join(dir, "ZEBRACODE.md"), projectRoot)
		add(&sources, seen, filepath.Join(dir, "AGENTS.md"), projectRoot)
	}
	add(&sources, seen, filepath.Join(workDir, ".zebracode", "INSTRUCTIONS.md"), projectRoot)
	add(&sources, seen, filepath.Join(workDir, "ZEBRACODE.local.md"), projectRoot)
	return sources
}
...
// 允许范围：项目目录（projectRoot）及其子目录，以及用户 home 目录下的 .zebracode/。
...
func isIncludeAllowed(absPath, projectRoot string) bool {
	...
	// 用户 home 下的 .zebracode/ 目录也允许（全局指令文件的 include）
	if home, err := os.UserHomeDir(); err == nil {
		zebracodeDir := filepath.Join(home, ".zebracode")
		if strings.HasPrefix(absPath, zebracodeDir+string(filepath.Separator)) {
			return true
		}
	}
	return false
