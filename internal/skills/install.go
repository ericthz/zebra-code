...
func (f *fetcher) listContents(src *SkillSource, subpath string) ([]contentEntry, error) {
	...
	req.Header.Set("User-Agent", "zebracode-install-skill")
...
// is expected to be the user-global skills tier (~/.zebracode/skills/) so
...
// UserSkillsRoot returns ~/.zebracode/skills, creating the parent if needed
...
func UserSkillsRoot() (string, error) {
	...
	root := filepath.Join(home, ".zebracode", "skills")
