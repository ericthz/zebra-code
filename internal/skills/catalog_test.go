...
func TestLoadCatalogPhase1EmptyBody(t *testing.T) {
	work := t.TempDir()
	writeSkillDir(t, filepath.Join(work, ".zebracode", "skills"), "demo",
...
func TestCatalogGetFullHotReload(t *testing.T) {
	work := t.TempDir()
	dir := writeSkillDir(t, filepath.Join(work, ".zebracode", "skills"), "hot",
...
func TestCatalogNeedsReload(t *testing.T) {
	work := t.TempDir()
	skillsDir := filepath.Join(work, ".zebracode", "skills")
