...
func TestLoadZebracodeSkills(t *testing.T) {
	...
	for wd != "/" {
		if _, err := os.Stat(filepath.Join(wd, ".zebracode", "skills")); err == nil {
			break
		}
		wd = filepath.Dir(wd)
	}
	...
	if _, err := os.Stat(skillsDir); os.IsNotExist(err) {
		t.Skip("No .zebracode/skills directory found")
	}
...
func TestLoadSkillsMergesPriority(t *testing.T) {
	work := t.TempDir()
	zebracodeDir := filepath.Join(work, ".zebracode", "skills", "shared")
	if err := os.MkdirAll(zebracodeDir, 0o755); err != nil {
		t.Fatal(err)
	}
	os.WriteFile(filepath.Join(zebracodeDir, "SKILL.md"), []byte(`---
name: shared
description: project skill from .zebracode
---
zebracode body`), 0o644)
	...
	if !strings.Contains(got.PromptBody, "zebracode body") {
		t.Errorf("expected zebracode body; got body=%q", got.PromptBody)
	}
}

func TestLoadSkillsForkMode(t *testing.T) {
	dir := t.TempDir()
	skillDir := filepath.Join(dir, ".zebracode", "skills", "reviewer")
	os.MkdirAll(skillDir, 0o755)
	os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte(`---
name: reviewer
...
func TestLoadSkillsContextFork(t *testing.T) {
	dir := t.TempDir()
	skillDir := filepath.Join(dir, ".zebracode", "skills", "forky")
	os.MkdirAll(skillDir, 0o755)
	os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte(`---
name: forky
