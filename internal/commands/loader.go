...
// LoadUserCommands merges file-based commands from two search paths: 1. ~/.zebracode/commands/ (user
// global) 2. $workDir/.zebracode/commands/ (project).
//
// Later sources override earlier ones on name collision.
func LoadUserCommands(workDir string) []*Command {
	var dirs []string
	if home, err := os.UserHomeDir(); err == nil {
		dirs = append(dirs, filepath.Join(home, ".zebracode", "commands"))
	}
	dirs = append(dirs,
		filepath.Join(workDir, ".zebracode", "commands"),
	)
