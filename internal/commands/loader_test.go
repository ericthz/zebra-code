...
func TestLoadUserCommandsMergesAndOverrides(t *testing.T) {
	work := t.TempDir()
	mewProj := filepath.Join(work, ".zebracode", "commands")
	mustWrite(t, filepath.Join(mewProj, "shared.md"), `from zebracode`)
	...
	if !strings.Contains(names["shared"], "from zebracode") {
		t.Errorf("expected 'from zebracode'; got %q", names["shared"])
	}
